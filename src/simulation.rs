...
use rand::Rng;
use crate::particle::{spawn_particle, GroupId, Locked, Particle, NeighborCount};
...
pub fn particle_locking_system(
    ...
    let entities: Vec<_> = query.iter_mut().map(|(e, v, l, g)| (e, v.linvel.length(), l.0, g.0)).collect();
    ...
    for (e1, e2) in locked_pairs {
        if let Ok([(_, mut _v1, mut l1, mut g1), (_, mut _v2, mut l2, mut g2)]) = query.get_many_mut([e1, e2]) {
            l1.0 = true;
            l2.0 = true;
            if g1.0 == 0 && g2.0 == 0 {
                g1.0 = group_counter;
...
pub fn user_input_system(
    ...
    if let Some(cursor_pos) = window.cursor_position() {
        let world_pos = Vec2::new(cursor_pos.x - window.width() / 2.0, cursor_pos.y - window.height() / 2.0);
...
pub fn neighbor_counting_system(
    ...
    let particles: Vec<(Entity, Vec2)> = query.iter().map(|(e, t, _)| (e, t.translation.truncate())).collect();
    
    // Count neighbors for each particle
    for i in 0..particles.len() {
        let (entity_i, pos_i) = particles[i];
        let mut count = 0;
        for j in 0..particles.len() {
            if i != j {
                let (_, pos_j) = particles[j];
                if (pos_i - pos_j).length() < neighbor_threshold {
                    count += 1;
                }
            }
        }
