...
pub fn rigid_body_formation_system(
    ...
    query: Query<(Entity, &Transform, &super::particle::GroupId, &super::particle::Locked, &super::particle::ParticleColor), With<super::particle::Particle>>,
) {
    ...
    for (entity, transform, group_id, locked, color) in query.iter() {
        if group_id.0 > 0 && locked.0 {
            ...
            groups.entry(group_id.0).or_insert_with(Vec::new).push((entity, pos, color.0));
        }
    }
    ...
    for (_group_id, particles) in groups {
        if particles.len() >= 3 {
            ...
            let center: Vec2 = particles.iter().map(|(_, pos, _)| pos).sum::<Vec2>() / particles.len() as f32;
            
            // Compute convex hull (simple gift wrapping algorithm)
            if let Some(hull) = compute_convex_hull(&particles) {
                // Compute bounding radius
                let bounding_radius = hull.iter()
                ...
                let _moment_of_inertia: f32 = particles.iter()
...
fn compute_convex_hull(particles: &[(Entity, Vec2, Color)]) -> Option<Vec<Vec2>> {
    ...
    for i in 1..points.len() {
        if points[i].x < points[min_idx].x || (points[i].x == points[min_idx].x && points[i].y < points[min_idx].y) {
            min_idx = i;
        }
    }
