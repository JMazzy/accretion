...
/// Original undeformed vertices in local space.
///
/// Used as the base shape for crater deformation.  The visual [`Vertices`] are
/// regenerated from these base vertices + accumulated craters.  Collider uses
/// base vertices (convex hull) for stable physics (Option A strategy).
#[derive(Component, Debug, Clone)]
pub struct BaseVertices(pub Vec<Vec2>);

/// Accumulated craters from repeated non-lethal projectile impacts.
///
/// Each crater is `(local_position, depth, radius)`.  The visual mesh is
/// regenerated by applying all craters to [`BaseVertices`] on each hit.
#[derive(Component, Debug, Clone, Default)]
pub struct CraterData {
    pub craters: Vec<(Vec2, f32, f32)>,
}
...
///
/// Always uses base vertices (from BaseVertices component) to create a convex
/// hull collider.  Visual mesh may have craters (concave), but physics stays
/// stable with the original convex hull (Option A strategy).
pub fn collider_for_vertices(vertices: &[Vec2]) -> Collider {
    if vertices.len() >= 3 {
        if let Some(c) = Collider::convex_hull(vertices) {
            c
        } else {
            eprintln!(
...
/// Apply crater-based deformation to base vertices using accumulated crater data.
///
/// Returns deformed vertices with craters applied.  Each crater subdivides
/// affected edges and displaces vertices inward with radial falloff.
pub fn apply_crater_deformation(
    base_vertices: &[Vec2],
    craters: &[(Vec2, f32, f32)],
    config: &PhysicsConfig,
) -> Option<Vec<Vec2>> {
    ...
    if craters.is_empty() {
        return Some(base_vertices.to_vec());
    }

    // Step 1: Subdivide edges near any crater
    let mut working_verts = base_vertices.to_vec();
    for &(crater_center, _depth, crater_radius) in craters.iter() {
        let mut new_verts = Vec::new();
        let n = working_verts.len();
        for i in 0..n {
            let v0 = working_verts[i];
            let v1 = working_verts[(i + 1) % n];
            new_verts.push(v0);

            let edge_midpoint = (v0 + v1) * 0.5;
            let dist_to_crater = edge_midpoint.distance(crater_center);
            if dist_to_crater < crater_radius * 1.5 {
                for j in 1..=config.crater_edge_subdivisions {
                    let t = j as f32 / (config.crater_edge_subdivisions + 1) as f32;
    ...
    // Step 2: Apply inward displacement with radial falloff for each crater
    let mut deformed = working_verts.clone();
    for &(crater_center, depth, crater_radius) in craters.iter() {
        for vertex in deformed.iter_mut() {
            let dist = vertex.distance(crater_center);
            if dist < crater_radius {
                let falloff = 1.0 - (dist / crater_radius).powi(2);
                let displacement = falloff * depth.min(config.crater_max_depth);
                let to_center = (crater_center - *vertex).normalize_or_zero();
                *vertex += to_center * displacement;
            }
        }
    }
...
pub fn spawn_initial_asteroids(commands: &mut Commands, count: usize, config: &PhysicsConfig) {
    ...
    for cell_idx in cell_indices {
        ...
        commands.spawn((
            ...
            (
                collider_for_vertices(&vertices),
...
pub fn spawn_orbit_scenario(commands: &mut Commands, config: &PhysicsConfig) {
    ...
    commands.spawn((
        (
            ...
            BaseVertices(central_vertices.clone()),
            CraterData::default(),
            RigidBody::Fixed,
        ),
        (
...
pub fn spawn_asteroid_with_vertices(
    ...
    let collider = collider_for_vertices(hull);
...
mod tests {
    ...
    #[test]
    fn collider_for_concave_polygon_stays_non_compound_option_a() {
        // Arrow-like concave polygon in local space.
        let concave = vec![
            Vec2::new(-4.0, -2.0),
            Vec2::new(2.0, -2.0),
            Vec2::new(2.0, -4.0),
        ...
        let collider = collider_for_vertices(&concave);
        assert!(
            collider.as_compound().is_none(),
            "Option A should keep a single convex collider even for concave visuals"
        );
    }

    #[test]
    fn collider_for_convex_polygon_stays_non_compound() {
        let convex = vec![
            Vec2::new(-3.0, -2.0),
            Vec2::new(3.0, -2.0),
            Vec2::new(4.0, 1.0),
        ...
        let collider = collider_for_vertices(&convex);
        assert!(
            collider.as_compound().is_none(),
    ...
    #[test]
    fn crater_deformation_no_craters_returns_base_shape() {
        let base = vec![
            Vec2::new(-10.0, -10.0),
            Vec2::new(10.0, -10.0),
            Vec2::new(10.0, 10.0),
        ...
        let out = apply_crater_deformation(&base, &[], &cfg).expect("base shape should be valid");
        assert_eq!(out, base);
    }

    #[test]
    fn crater_deformation_subdivides_edges_near_crater_zone() {
        let base = vec![
            Vec2::new(-10.0, -10.0),
            Vec2::new(10.0, -10.0),
            Vec2::new(10.0, 10.0),
        ...
        let cfg = PhysicsConfig::default();
        let craters = vec![(Vec2::new(0.0, 10.0), 1.0, 6.0)];

        let out =
            apply_crater_deformation(&base, &craters, &cfg).expect("deformation should succeed");
        assert!(
            out.len() > base.len(),
            "expected local subdivision to add vertices near crater zone"
        );
    }

    #[test]
    fn crater_deformation_depth_is_clamped_to_config_max() {
        let base = vec![
            Vec2::new(-10.0, -10.0),
            Vec2::new(10.0, -10.0),
            Vec2::new(10.0, 10.0),
        ...
        let at_max =
            apply_crater_deformation(&base, &[(center, cfg.crater_max_depth, radius)], &cfg)
                .expect("deformation at max depth should succeed");
        let above_max = apply_crater_deformation(
            &base,
            &[(center, cfg.crater_max_depth + 50.0, radius)],
            &cfg,
        )
        .expect("deformation above max depth should succeed");

        assert_eq!(
            at_max, above_max,
            "depth above max should clamp to crater_max_depth"
        );
    }

    #[test]
    fn crater_deformation_rejects_degenerate_input() {
        let cfg = PhysicsConfig::default();
        let degenerate = vec![Vec2::ZERO, Vec2::X];
        let out = apply_crater_deformation(&degenerate, &[(Vec2::ZERO, 1.0, 5.0)], &cfg);
        assert!(out.is_none(), "fewer than 3 base vertices must be rejected");
    }

    #[test]
    fn crater_deformation_overlapping_impacts_is_stable_and_deterministic() {
        let base = vec![
            Vec2::new(-12.0, -12.0),
            Vec2::new(12.0, -12.0),
            Vec2::new(12.0, 12.0),
        ...
        let cfg = PhysicsConfig::default();
        let craters = vec![
            (Vec2::new(-1.5, 10.0), 1.2, 7.0),
            (Vec2::new(-0.5, 10.2), 1.4, 7.0),
            (Vec2::new(0.5, 9.8), 1.1, 7.0),
            (Vec2::new(1.5, 10.1), 1.3, 7.0),
            (Vec2::new(0.0, 10.0), 1.5, 7.0),
        ];
        ...
        assert_eq!(
            out_a, out_b,
            "same input crater set should produce deterministic output"
        );
        assert!(
            out_a.iter().all(|v| v.x.is_finite() && v.y.is_finite()),
            "deformed vertices must remain finite"
        );
        assert!(
            out_a.len() > base.len(),
            "overlapping craters should add local vertex detail"
        );
        assert!(
            out_a.len() < 5_000,
            "vertex growth should remain bounded in this regression case (got {})",
            out_a.len()
        );
        assert!(
